<script lang="ts">
	...
	const placeholder: boolean = getContext('media-avatar-placeholder');

	const defaultClass =
		'absolute text-content inset-0 h-full w-full flex items-center justify-center overflow-hidden bg-default stwui-media-avatar-placeholder';
	const circleClass = 'rounded-full';
	const roundedClass = 'rounded-md';

	const defaultIconContainerClass =
		'absolute text-content h-full w-full stwui-media-avatar-placeholder-wrapper';
